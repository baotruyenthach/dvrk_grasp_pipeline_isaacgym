...
class GenGraspPreshape():
    ...
    #     # rot_matrix = np.matrix([y_axis, x_axis, z_axis]).T
    ...
    #     # roll = np.random.uniform(-np.pi, np.pi)
    #     roll = 1/6*np.pi
    #     x_rand = np.array([0.0, np.sin(roll), np.cos(roll)])
    ...
    #     quaternion = tf.transformations.quaternion_from_matrix(trans_matrix)
    #     rospy.loginfo('Grasp orientation = ' +  str(quaternion))
    #     rospy.loginfo('Grasp orientation in rotation matrix = ' +  str(tf.transformations.quaternion_matrix(quaternion)))
    #     rospy.loginfo('Grasp orientation in euler angles = ' +  str(tf.transformations.euler_from_quaternion(quaternion)))
    #     return quaternion
